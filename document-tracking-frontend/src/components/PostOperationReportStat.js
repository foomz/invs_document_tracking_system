...
const PostOperationReportStat = () => {
  const [data, setData] = useState(initialData);

  useEffect(() => {
    axios.get('http://localhost:8000/api/PostOperationReportStat/')
      .then(response => {
  ...
  const handleBlur = (region, month, value) => {
    ...
    axios.post('http://localhost:8000/api/PostOperationReportStat/', payload)
      .catch(error => {
...
export default PostOperationReportStat;
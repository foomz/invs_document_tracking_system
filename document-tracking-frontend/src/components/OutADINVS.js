...
function OutADINVS() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8000/api/outadinvs/')
      .then(response => {
  ...
  const handleSave = (id) => {
    ...
    axios.put(`http://localhost:8000/api/outadinvs/${id}/`, document)
      .then(response => {
        setOutgoingDocuments(prevState =>
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8000/api/outadinvs/${id}/`)
      .then(() => {

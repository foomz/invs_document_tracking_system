...
function PostOperationReport() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8000/api/PostOperationReport/')
      .then(response => {
  ...
  const handleSave = (id) => {
    ...
    axios.put(`http://localhost:8000/api/PostOperationReport/${id}/`, item)
      .then(response => {
        setItems(prevState =>
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8000/api/PostOperationReport/${id}/`)
      .then(() => {
  ...
  const handleNewItemSave = () => {
    axios.post('http://localhost:8000/api/PostOperationReport/', newItem)
      .then(response => {
              ...
              <td>
                {editMode === item.id ? (
                  <input
                    ...
                    value={item.disposition_result}
                    onChange={(e) => handleChange(e, item.id)}
                  />
                ) : (
                  item.disposition_result
                )}
              </td>
              <td>
                {editMode === item.id ? (
          ...
          <tr>
            ...
            <td>
              <input
                type="text"
                name="disposition_result"
                value={newItem.disposition_result}
                onChange={handleNewItemChange}
              />
            </td>
            <td>
...
export default PostOperationReport;